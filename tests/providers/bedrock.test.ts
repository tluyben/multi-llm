...
const hasCredentials = checkEnvironmentVariables('bedrock');

const describeConditional = hasCredentials ? describe : describe.skip;

describeConditional('Bedrock Provider Tests', () => {
  ...
  beforeAll(() => {
    provider = MultiLLM.createProvider('bedrock', apiKey, region) as BedrockProvider;
  });

  describe('Provider Creation', () => {
    test('should create Bedrock provider successfully', () => {
  ...
  describe('Non-Streaming Chat', () => {
    ...
    test('should handle simple chat request', async () => {
      ...
      const result = await llm.chat('Say "Hello, Amazon Bedrock!" and nothing else.', {
        temperature: 0.1,
        maxTokens: 50
      });
  ...
  describe('Error Handling', () => {
    test('should handle invalid model gracefully', async () => {
      const invalidProvider = MultiLLM.createProvider('bedrock', apiKey, region) as BedrockProvider;
    ...
    test('should handle empty API key', () => {
      expect(() => {
        MultiLLM.createProvider('bedrock', '', region);
