...
import { DeepInfraProvider } from '../../src/providers/deepinfra';
import { checkEnvironmentVariables, TEST_CONFIG } from '../setup';

const hasCredentials = checkEnvironmentVariables('deepinfra');

const describeConditional = hasCredentials ? describe : describe.skip;

describeConditional('DeepInfra Provider Tests', () => {
  let provider: DeepInfraProvider;
  const apiKey = process.env.DEEPINFRA_API_KEY!;
  const modelId = process.env.DEEPINFRA_MODEL!;

  beforeAll(() => {
    provider = MultiLLM.createProvider('deepinfra', apiKey) as DeepInfraProvider;
  });

  describe('Provider Creation', () => {
    test('should create DeepInfra provider successfully', () => {
  ...
  describe('Non-Streaming Chat', () => {
    ...
    test('should handle simple chat request', async () => {
      ...
      const result = await llm.chat('Say "Hello, DeepInfra!" and nothing else.', {
        temperature: 0.1,
        maxTokens: 50
      });
  ...
  describe('Error Handling', () => {
    test('should handle invalid model gracefully', async () => {
      const invalidProvider = MultiLLM.createProvider('deepinfra', apiKey) as DeepInfraProvider;
    ...
    test('should handle empty API key', () => {
      expect(() => {
        MultiLLM.createProvider('deepinfra', '');
