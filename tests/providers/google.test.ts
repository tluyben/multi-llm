...
const hasCredentials = checkEnvironmentVariables('google');

const describeConditional = hasCredentials ? describe : describe.skip;

describeConditional('Google Provider Tests', () => {
  let provider: GoogleProvider;
  const apiKey = process.env.GOOGLE_API_KEY!;
  const modelId = process.env.GOOGLE_MODEL!;

  beforeAll(() => {
    provider = MultiLLM.createProvider('google', apiKey) as GoogleProvider;
  });
  ...
  describe('Non-Streaming Chat', () => {
    ...
    test('should handle simple chat request', async () => {
      ...
      const result = await llm.chat('Say "Hello, Google Gemini!" and nothing else.', {
        temperature: 0.1,
        maxTokens: 50
      });
  ...
  describe('Error Handling', () => {
    test('should handle invalid model gracefully', async () => {
      const invalidProvider = MultiLLM.createProvider('google', apiKey) as GoogleProvider;
    ...
    test('should handle empty API key', () => {
      expect(() => {
        MultiLLM.createProvider('google', '');
