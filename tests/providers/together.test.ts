...
const hasCredentials = checkEnvironmentVariables('together');

const describeConditional = hasCredentials ? describe : describe.skip;

describeConditional('Together Provider Tests', () => {
  let provider: TogetherProvider;
  const apiKey = process.env.TOGETHER_API_KEY!;
  const modelId = process.env.TOGETHER_MODEL!;

  beforeAll(() => {
    provider = MultiLLM.createProvider('together', apiKey) as TogetherProvider;
  });

  describe('Provider Creation', () => {
    test('should create Together provider successfully', () => {
  ...
  describe('Non-Streaming Chat', () => {
    ...
    test('should handle simple chat request', async () => {
      ...
      const result = await llm.chat('Say "Hello, Together!" and nothing else.', {
        temperature: 0.1,
        maxTokens: 50
      });
  ...
  describe('Error Handling', () => {
    test('should handle invalid model gracefully', async () => {
      const invalidProvider = MultiLLM.createProvider('together', apiKey) as TogetherProvider;
    ...
    test('should handle empty API key', () => {
      expect(() => {
        MultiLLM.createProvider('together', '');
