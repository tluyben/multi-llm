...
const hasCredentials = checkEnvironmentVariables('perplexity');

const describeConditional = hasCredentials ? describe : describe.skip;

describeConditional('Perplexity Provider Tests', () => {
  let provider: PerplexityProvider;
  const apiKey = process.env.PERPLEXITY_API_KEY!;
  const modelId = process.env.PERPLEXITY_MODEL!;

  beforeAll(() => {
    provider = MultiLLM.createProvider('perplexity', apiKey) as PerplexityProvider;
  });

  describe('Provider Creation', () => {
    test('should create Perplexity provider successfully', () => {
  ...
  describe('Non-Streaming Chat', () => {
    ...
    test('should handle simple chat request', async () => {
      ...
      const result = await llm.chat('Say "Hello, Perplexity!" and nothing else.', {
        temperature: 0.1,
        maxTokens: 50
      });
  ...
  describe('Error Handling', () => {
    test('should handle invalid model gracefully', async () => {
      const invalidProvider = MultiLLM.createProvider('perplexity', apiKey) as PerplexityProvider;
    ...
    test('should handle empty API key', () => {
      expect(() => {
        MultiLLM.createProvider('perplexity', '');
