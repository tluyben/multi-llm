...
const hasCredentials = checkEnvironmentVariables('fireworks');

const describeConditional = hasCredentials ? describe : describe.skip;

describeConditional('Fireworks Provider Tests', () => {
  let provider: FireworksProvider;
  const apiKey = process.env.FIREWORKS_API_KEY!;
  const modelId = process.env.FIREWORKS_MODEL!;

  beforeAll(() => {
    provider = MultiLLM.createProvider('fireworks', apiKey) as FireworksProvider;
  });

  describe('Provider Creation', () => {
    test('should create Fireworks provider successfully', () => {
  ...
  describe('Non-Streaming Chat', () => {
    ...
    test('should handle simple chat request', async () => {
      ...
      const result = await llm.chat('Say "Hello, Fireworks!" and nothing else.', {
        temperature: 0.1,
        maxTokens: 50
      });
  ...
  describe('Error Handling', () => {
    test('should handle invalid model gracefully', async () => {
      const invalidProvider = MultiLLM.createProvider('fireworks', apiKey) as FireworksProvider;
    ...
    test('should handle empty API key', () => {
      expect(() => {
        MultiLLM.createProvider('fireworks', '');
