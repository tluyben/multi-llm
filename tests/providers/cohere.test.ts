...
const hasCredentials = checkEnvironmentVariables('cohere');

const describeConditional = hasCredentials ? describe : describe.skip;

describeConditional('Cohere Provider Tests', () => {
  let provider: CohereProvider;
  const apiKey = process.env.COHERE_API_KEY!;
  const modelId = process.env.COHERE_MODEL!;

  beforeAll(() => {
    provider = MultiLLM.createProvider('cohere', apiKey) as CohereProvider;
  });

  describe('Provider Creation', () => {
    test('should create Cohere provider successfully', () => {
    ...
    test('should have correct base configuration', () => {
      ...
      expect(provider['baseUrl']).toBe('https://api.cohere.ai/v1');
    });
  });
  ...
  describe('Non-Streaming Chat', () => {
    ...
    test('should handle simple chat request', async () => {
      ...
      const result = await llm.chat('Say "Hello, Cohere!" and nothing else.', {
        temperature: 0.1,
        maxTokens: 50
      });
  ...
  describe('Error Handling', () => {
    test('should handle invalid model gracefully', async () => {
      const invalidProvider = MultiLLM.createProvider('cohere', apiKey) as CohereProvider;
    ...
    test('should handle empty API key', () => {
      expect(() => {
        MultiLLM.createProvider('cohere', '');
